 ...
 *
 * We use a mutex (not spinlock) because get_rss_bytes() calls
 * get_task_mm()/mmput() which can sleep, and the timer callback
 * runs in a sleepable context via workqueue-backed timers.
 * ================================================================ */
...
/* ================================================================
 * RSS helper — returns bytes, or -1 if task gone
 * ================================================================ */
                         ...
                         unsigned long limit_bytes, long rss_bytes)
{
    ...
    rcu_read_unlock();
    printk(KERN_WARNING
           "[container_monitor] HARD LIMIT container=%s pid=%d rss=%ld limit=%lu — sent SIGKILL\n",
           container_id, pid, rss_bytes, limit_bytes);
}
...
static void timer_callback(struct timer_list *t)
{
    ...
    list_for_each_entry_safe(entry, tmp, &monitored_list, list) {
        ...
        if (rss < 0) {
            printk(KERN_INFO "[container_monitor] Stale entry removed: container=%s pid=%d\n",
...
static long monitor_ioctl(struct file *f, unsigned int cmd, unsigned long arg)
{
    ...
    if (cmd == MONITOR_REGISTER) {
        ...
        if (req.soft_limit_bytes > req.hard_limit_bytes) {
            printk(KERN_WARNING "[container_monitor] Register rejected: soft > hard for %s\n",
                   req.container_id);
            return -EINVAL;
        }
        ...
        e->soft_warned      = 0;
        strncpy(e->container_id, req.container_id, MONITOR_NAME_LEN - 1);
        e->container_id[MONITOR_NAME_LEN - 1] = '\0';
        INIT_LIST_HEAD(&e->list);

        mutex_lock(&monitored_lock);
        list_add(&e->list, &monitored_list);
        mutex_unlock(&monitored_lock);
